function [Sy,Hsy,Hmy,Hiy,Hzy] = Sy2d2(mode,u,tau)
% param2D;
param;
...
for i=1:1:max(m)
    Sy(1,1,i)=-k1*Hi(i)-Hm(i)+Fy11*Hs(i)-v11*Hz(i);
    Sy(1,2,i)=k2*Hi(i)+Fy12*Hs(i)+v12*Hz(i);
    Sy(2,1,i)=k2*Hi(i)+Fy21*Hs(i)+v21*Hz(i);
    Sy(2,2,i)=-k1*Hi(i)-Hm(i)+Fy22*Hs(i)+v22*Hz(i);
end
% for i=1:1:max(m)
%     Sy(1,1,i)=-k1*H_iy(i)-H_my(i)+Fy11*H_sy(i)-v11*H_zy(i);
%     Sy(1,2,i)=k2*H_iy(i)+Fy12*H_sy(i)+v12*H_zy(i);
%     Sy(2,1,i)=k2*H_iy(i)+Fy21*H_sy(i)+v21*H_zy(i);
%     Sy(2,2,i)=k1*H_iy(i)-H_my(i)+Fy22*H_sy(i)+v22*H_zy(i);
% end
end
